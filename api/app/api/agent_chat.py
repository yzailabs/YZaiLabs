...
import re
from app.core.onchain import transfer_bnb
...
async def agent_chat(
    ...
    handled = True
    transfer_patterns = [
        r"send\s+([\d.]+)\s+bnb\s+to\s+(me|owner|0x[a-fA-F0-9]{40})",
        r"send\s+(me|owner|0x[a-fA-F0-9]{40})\s+([\d.]+)\s+bnb",
    ]
    ...
    if transfer_match:
        ...
        if target in ["me", "owner"]:
            ...
            # Direct address → require confirmation
            if "confirm" not in human_message_lower:
                return {
                    "agent_response": (
                        f"You are about to send {amount} BNB to external address {target}.\n\n"
                        f"Type 'confirm transfer' to proceed."
                    )
                }
    ...
    if any(k in human_message_lower for k in presale_keywords):
        ...
        action = AgentAction(
            ...
            agent_name=agent.name,
            agent_address=agent.wallet_address,
            action=f"Participated in presale {presale.token_address} with {amount} BNB"
        )
        db.add(action)
        await db.commit()
